...
if __name__ == "__main__":
    ...
    # We generate 3 additional additional CSV training files to train 3 different
    ...
    bands = ['CoastalBlue', 'Blue', 'Green', 'Yellow', 'Red', 'RedEdge', 'NIR1', 'NIR2', 'DVI', 'FDI', 'SI', 'Class']
    df = pd.read_csv(csvfile, header=None, sep=',', names=bands)  # generate pd dataframe

    # Case #1: cloud_training_8band_fdi_si_ndwi.csv
    # training data using all 8 bands from imagery and the 3 indices calculated using all of the bands.
    fbands = ['CoastalBlue', 'Blue', 'Green', 'Yellow', 'Red', 'RedEdge', 'NIR1', 'NIR2', 'FDI', 'SI', 'NDWI', 'Class']
    df1 = df.drop(columns=['DVI', 'FDI', 'SI'])

    df1['FDI'] = df1.apply(lambda row: row.NIR2 - (row.RedEdge + row.Blue), axis=1)
    df1['SI'] = df1.apply(lambda row: ((10000 - row.Blue) * (10000 - row.Green) * (10000 - row.Red)) ** (1.0/3), axis=1)
    df1['NDWI'] = df1.apply(lambda row: 10000 * (row.Green - row.NIR1) / (row.Green + row.NIR1), axis=1)
    df1 = df1.fillna(0)
    df1 = df1.astype({'FDI': 'int16', 'SI': 'int16', 'NDWI': 'int16'})

    df1 = df1[fbands]
    print("shape df: ", df1, df1.shape)
    df1.to_csv('cloud_training_8band_fdi_si_ndwi.csv', index=False, header=False)

    # Case #2: cloud_training_4band_fdi_si_ndwi.csv
    # training data using only 4 bands from imagery and the 3 indices calculated using only 4 bands.
    # The order of the bands goes accordingly to 8 band imagery (B-G-R-NIR).
    fbands = ['Blue', 'Green', 'Red', 'NIR1', 'FDI', 'SI', 'NDWI', 'Class']
    df2 = df.drop(columns=['CoastalBlue', 'Yellow', 'RedEdge', 'NIR2', 'DVI', 'FDI', 'SI'])

    df2['FDI'] = df2.apply(lambda row: row.NIR1 - (row.Red + row.Blue), axis=1)
    df2['SI'] = df2.apply(lambda row: ((10000 - row.Blue) * (10000 - row.Green) * (10000 - row.Red)) ** (1.0/3), axis=1)
    df2['NDWI'] = df2.apply(lambda row: 10000 * (row.Green - row.NIR1) / (row.Green + row.NIR1), axis=1)
    df2 = df2.fillna(0)
    df2 = df2.astype({'FDI': 'int16', 'SI': 'int16', 'NDWI': 'int16'})

    df2 = df2[fbands]
    print("shape df: ", df2, df2.shape)
    df2.to_csv('cloud_training_4band_fdi_si_ndwi.csv', index=False, header=False)
    ...
    df3.to_csv('cloud_training_4band_rgb_fdi_si_ndwi.csv', index=False, header=False)
