...
set -euo pipefail
out=${1:-results/prism-scaling-$(date -u +%Y%m%dT%H%M%SZ)}
if [[ $# -gt 0 ]]; then shift;fi
work=${TERRAGPU_IO_ROOT:-data/scaling}
...
stage=correctness
if ! python -m pytest tests/test_publication_benchmark.py tests/test_scaling_benchmark.py -q --tb=short --disable-warnings > "$out/tests.txt" 2>&1;then
 tail -60 "$out/tests.txt";exit 1
fi
stage='scene-size scaling sweep'
python -m terragpu.scaling_benchmark --output "$out/sweep" --work-root "$work" --data-root "$data_root" --repeat "${TERRAGPU_REPEAT:-5}" --seed "${TERRAGPU_SEED:-731}" --storage-label "${TERRAGPU_STORAGE_LABEL:-unspecified}" "$@"
echo "Completed: $out. Archive this directory; generated scenes and scratch outputs were removed."
